...
`)
		...
		Describe("waitForReady=false skips readiness check", func() {
			It("should proceed immediately when waitForReady is disabled", func() {
				By("Given a LynqForm with waitForReady=false on a resource that would normally fail")
				createForm(formName, hubName, `
  deployments:
    - id: no-wait-deployment
      nameTemplate: "{{ .uid }}-no-wait-deploy"
      waitForReady: false
      timeoutSeconds: 30
      spec:
        apiVersion: apps/v1
...
`)
				...
				By("Then both resources should be created quickly (no waiting for deployment ready)")
				deploymentName := fmt.Sprintf("%s-no-wait-deploy", uid)
				configMapName := fmt.Sprintf("%s-after-config", uid)

				// Both should exist relatively quickly since we're not waiting for ready
				Eventually(func(g Gomega) {
					cmd := exec.Command("kubectl", "get", "deployment", deploymentName, "-n", policyTestNamespace)
					_, err := utils.Run(cmd)
					g.Expect(err).NotTo(HaveOccurred())
				...
				By("And the ConfigMap should have been created despite deployment not being ready")
				cmd := exec.Command("kubectl", "get", "configmap", configMapName, "-n", policyTestNamespace,
					"-o", "jsonpath={.data.key}")
				output, err := utils.Run(cmd)
				Expect(err).NotTo(HaveOccurred())
				Expect(output).To(Equal("should-be-created"))
			})
		})

		Describe("Dependent resource skipped on failure", func() {
			It("should not create dependent resource when dependency times out", func() {
				By("Given a LynqForm where resource-b depends on resource-a, and resource-a will timeout")
				createForm(formName, hubName, `
  deployments:
    - id: resource-a
      nameTemplate: "{{ .uid }}-dep-a"
      waitForReady: true
      timeoutSeconds: 20
      spec:
        apiVersion: apps/v1
        ...
        spec:
          ...
          template:
            ...
            spec:
              containers:
              - name: app
                image: non-existent-registry.example.com/will-timeout:v1
  configMaps:
    - id: resource-b
      nameTemplate: "{{ .uid }}-dep-b"
      dependIds:
        - resource-a
      spec:
        apiVersion: v1
...
`)
				...
				deploymentName := fmt.Sprintf("%s-dep-a", uid)
				Eventually(func(g Gomega) {
					cmd := exec.Command("kubectl", "get", "deployment", deploymentName, "-n", policyTestNamespace)
					_, err := utils.Run(cmd)
					g.Expect(err).NotTo(HaveOccurred())
				...
				By("And resource-b (ConfigMap) should NOT be created (dependency failed)")
				configMapName := fmt.Sprintf("%s-dep-b", uid)
				// Wait sufficient time for the timeout to occur and reconciliation to complete
				...
				By("And the LynqNode should have Ready=False")
				Eventually(func(g Gomega) {
					cmd := exec.Command("kubectl", "get", "lynqnode", expectedNodeName, "-n", policyTestNamespace,
						"-o", "jsonpath={.status.conditions[?(@.type=='Ready')].status}")
					output, err := utils.Run(cmd)
...
`)
		...
		Describe("ConfigMap immediate readiness", func() {
			It("should mark ConfigMap as ready immediately after creation", func() {
				By("Given a LynqForm with ConfigMap (should be immediately ready)")
				createForm(formName, hubName, `
  configMaps:
    - id: instant-config
      nameTemplate: "{{ .uid }}-instant-config"
      waitForReady: true
      timeoutSeconds: 10
      spec:
        apiVersion: v1
...
`)
				...
				configMapName := fmt.Sprintf("%s-instant-config", uid)
				Eventually(func(g Gomega) {
					cmd := exec.Command("kubectl", "get", "configmap", configMapName, "-n", policyTestNamespace)
					_, err := utils.Run(cmd)
					g.Expect(err).NotTo(HaveOccurred())
				...
				By("And the LynqNode should be Ready quickly")
				Eventually(func(g Gomega) {
					cmd := exec.Command("kubectl", "get", "lynqnode", expectedNodeName, "-n", policyTestNamespace,
						"-o", "jsonpath={.status.conditions[?(@.type=='Ready')].status}")
					output, err := utils.Run(cmd)
				...
				By("And status should show correct resource counts")
				Eventually(func(g Gomega) {
					cmd := exec.Command("kubectl", "get", "lynqnode", expectedNodeName, "-n", policyTestNamespace,
